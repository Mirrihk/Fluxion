    ...
    public readonly struct Graph1D
    {
            ...
            int resolution = 120, bool wireframe = false)
        {
            ...
            Func<double, double, double> g = lift switch
            {
                ...
                Lift.Radial => (x, y) => fLocal(System.Math.Sqrt(x * x + y * y)),
                Lift.ProductCos => (x, y) => fLocal(x) * System.Math.Cos(y),
                Lift.ProductSin => (x, y) => fLocal(x) * System.Math.Sin(y),
                _ => (x, y) => fLocal(x)
            };
    ...
    public sealed class GraphPoints
    {
        ...
        public void ThreeDLine(EmbedPlane plane = EmbedPlane.XY, double offset = 0)
        {
            ...
            Graph3DFeature.Parametric(
                t =>
                {
                    int i = (int)System.Math.Clamp(System.Math.Floor(t), 0, pts.Length - 1);
                    int j = System.Math.Min(i + 1, pts.Length - 1);
                    double a = t - i;
                    var (x0, y0) = pts[i];
                    var (x1, y1) = pts[j];
                    double x = x0 + a * (x1 - x0);
                ...
                tMin: 0,
                tMax: System.Math.Max(1, pts.Length - 1),
                samples: System.Math.Max(pts.Length, 512));
        }
    }
}
